...
| 3a | `fix(block-4): correct import depth in slack/oauth/start.js`                             | **Inserted between commits 3 and 4 as a plan amendment.** start.js used 5 `../` segments in its `_lib/` imports; correct is 4 (matches the working depth in `functions/api/projects/[id]/connections/index.js`). Bug surfaced during commit 4 design-pass review; preview build for commit 3 failed because of it. Commit 9's reservation framing still applies for any further fix-ups during commits 4–8 review. Total commit count: 11 + closeout. |
...
| 7a | `fix(block-4): correct import depth in slack/events.js`                                  | **Inserted between commits 7 and 8 as a plan amendment** (second of the kind in Block 4 — see also 3a). events.js used 4 `../` segments in its `_lib/` import; correct is 3. The file is at depth 4 (`functions/api/connectors/slack/`), not 5 like the oauth/ subdirectory which adds another level. Bug surfaced via Cloudflare Pages build failure (esbuild "Could not resolve" error in the build log) which kept commit 7 from deploying. Total commit count: 12 + closeout. **Cumulative pattern**: directory-depth miscounts caused both 3a and 7a; Block 9 polish opportunity is a pre-push smoke check (e.g., `wrangler pages dev` build) that would catch import-resolution errors before they hit Cloudflare Pages. |
