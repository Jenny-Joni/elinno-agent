...
_None — all fixes have landed on main. Branches deleted (local + remote) after each merge._

**Cumulative main commit progression this session:**
`bd47074` (v1.4 ship) → `4c5b3ba` (D10) → `5e40333` (D1+D2+D3+D6+D12) → `d0a171d` (D4+D5+D9). **8 fixes total** across 4 prod deploys.
...
| D4 | §0.5 | S0.5.15 | med | `/workspace_settings.html` top nav was missing the "Dashboard" link. Bonus: nav also showed email-inline instead of avatar; LOG OUT was primary-filled; logout handler called `/api/auth/logout` (a 404 endpoint). | **FIXED-PROD** | Cherry-picked to main as `42d0395`, pushed ~15:00 IDT. Re-verified on prod: Dashboard link present, avatar circle visible, LOG OUT ghost-styled. |
| D5 | §0.5 | S0.5.15 | low | Same page LOG OUT styling inconsistency (primary-filled vs ghost-outline). | **FIXED-PROD** | Bundled in `42d0395`. |
...
| D9 | §12 | S12.5 | low | `/project/foo/bar/baz` was returning 200 with login-page HTML instead of 404 (Pages SPA-fallback). | **FIXED-PROD** | Cherry-picked to main as `d0a171d`, pushed ~15:00 IDT. Adds `public/404.html` (v1.4 brand-styled). Verified on prod: same URL now returns **HTTP 404** with the new branded page (title "Not found — Elinno Agent"). |
