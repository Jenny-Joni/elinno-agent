...
- **Fix branches created during this run:**
  ...
  - `qa-fix-d6-next-redirect` (`39abf51` → `d16570d` after extension) — **FULL close of D6**. Initial commit covered dashboard / projects / project.html / login.html / index.html; extension added projects/new.html / admin.html / project_settings.html. Audit found cross-project/* pages already correctly preserved ?next= via their existing redirectToLogin() helpers — no change needed there. Awaits main-push approval.
...
2 new fix branches created during the extended-QA close-out:

1. **`qa-fix-d4-d5-workspace-nav`** (`6214128`) — closes D4 + D5 + an implicit nav-consistency drift + a latent logout-endpoint bug on workspace_settings.html.
2. **`qa-fix-d9-404-page`** (`c27ab57`) — adds `public/404.html` so Cloudflare Pages serves a real 404 for unknown URLs instead of falling through to the landing/login page.
...
| D4 | §0.5 | S0.5.15 | med | `/workspace_settings.html` top nav is missing the "Dashboard" link (only Cross-project / Projects / Admin shown). Users on this page can't return to dashboard via nav. Page hasn't been v1.4-reskinned (Phase 9 carry-forward, expected per HANDOFF). | FIXED-preview | `qa-fix-d4-d5-workspace-nav` (6214128) — awaits main-push. Adds Dashboard link, switches to avatar circle, ghost-styles Log out. Bonus: fixes latent logout bug (`/api/auth/logout` 404 → `/api/logout`). |
| D5 | §0.5 | S0.5.15 | low | `/workspace_settings.html` LOG OUT button is styled as a primary filled (lavender) button, while every other page uses a ghost-outline Log out. Style inconsistency. Same Phase 9 carry-forward as D4. | FIXED-preview | Same branch as D4 (`qa-fix-d4-d5-workspace-nav` / 6214128). |
...
| D8 | §3 | S3.1 | — | `/api/workspace` returns `user_count: 1` while D1 has 4 users. Investigated `functions/api/workspace.js:112` — value is **hardcoded by design**. Comment at file header (lines 7-9) documents: "v1.3 workspace model is solo: workspace = D1 user (per BLOCK_12_PLAN decision E). When a real `workspaces` table lands in v2.0, this endpoint is the natural seam to migrate." Semantic: `user_count` = number of WORKSPACES (always 1 in solo plan), not number of D1 user rows. **NOT A BUG** — QA.md expectation was wrong. | **RESOLVED (by-design)** | — |
| D9 | §12 | S12.5 | low | `/project/foo/bar/baz` returns 200 with login-page HTML instead of 404. Caused by Cloudflare Pages SPA-fallback config, NOT slug-routing dynamic function (the function correctly didn't fire). Affects any unknown deep URL on the site. UX nit. | FIXED-preview | `qa-fix-d9-404-page` (c27ab57) — awaits main-push. Adds `public/404.html` (v1.4 brand-styled) which Cloudflare Pages auto-serves for unknown URLs. Includes ?next= preservation for signed-out users. |
...
| D13 | §6 | S6.5/S6.7 | — | Block 9.5 contract: LLM answered "**96 open Jira tickets**" on Rain. Verified via Neon SQL (14:05): `COUNT(*) WHERE source='jira' AND project_id='<rain>' AND metadata->>'status_category' != 'done'` = **96** ✓. Number is real, not invented. **D13a follow-up investigation**: `citations` field is null because `functions/_lib/ai/loop.js:504-524` only extracts citations from tool results that have `entity_id` (specific records). Aggregate-count tools (COUNT(*) = 96) don't return per-row entities → no citations array population. **This is by design** — citations cite specific entities; aggregate query results cite the query itself, which is visible in the conversation's tool-call messages. Block 9.5 contract holds: no model-invented numbers, every number traces to a tool call in the conversation. | **RESOLVED (by-design)** | — |
