...
- **IDE markdown-formatter policy: RESOLVED 2026-05-03.** Three format-on-focus occurrences across Block 2 Sessions 1 and 2 (BLOCK_2_PLAN.md, DESIGN.md, SETUP.md — each caught and reverted pre-merge). Chose Option 2: disabled format-on-save / format-on-focus for `.md` files in IDE local config. No project-level Prettier or markdownlint. If the formatter ever fires again on `.md`, the IDE setting has regressed.
